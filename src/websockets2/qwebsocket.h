...
public:
    ...
    /*
    * Meaning of this enum / flags is to optimize incomming data,
    *
    * If someone wants to use the easy api (default) than receiveSignals method is set to: TextAndDataReceivedSignals
    * in this case, the signals: textReceived() & dataReceived() will be emitted accordingly on incomming data.
    * incomming frames will be queued, until the full message is received.
    *
    * In case someone wants to do stream reading (low level api), you could set receiveSignals to TextAndDataFrameReceivedSignals,
    * this way messages will not be queued, and every incomming frame will be directly emited.
    ...
    *
    * -> text message frames will be queued, untill the message is complete, and than emited trough:
